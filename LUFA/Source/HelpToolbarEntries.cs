...
namespace FourWalledCubicle.LUFA
{
    class HelpToolbarEntries
    {
        ...
        private void AddToolbarButtonHandler(int commandID, EventHandler callback)
        {
            ...
            {
                Logging.Log(Logging.Severity.Error, "Could not create toolbar entry for command {0}", commandID);
                return;
            }
            ...
            {
                Logging.Log(Logging.Severity.Error, "Could not create toolbar menu entry for command {0}", commandID);
                return;
            }
